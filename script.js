...
// Глобальная функция склонения слова "награда"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    setTimeout(() => {
        console.log('⏰ Запуск initApp через 100мс');
        initApp();
    }, 100);
    ...
    function initApp() {
        ...
        const currentUser = window.fitnessDB.getCurrentUser();
        console.log('👤 Текущий пользователь из сессии:', currentUser);
    ...
    function checkAndShowCompletionNotification() {
        // Уведомление о награде за челлендж
    ...
    function calculateProgress(user) {
        ...
        // Сколько тренировок нужно для следующего уровня
        // 1 уровень: 20 тренировок
        // 2 уровень: 30 тренировок
        // 3 уровень: 40 тренировок
        // Каждый уровень требует на 10 тренировок больше
        const trainingsNeededForNextLevel = 20 + (currentLevel - 1) * 10;

        // Тренировки, выполненные в текущем уровне
        let trainingsInCurrentLevel = trainings;

        // Вычитаем тренировки, потраченные на предыдущие уровни
        for (let i = 1; i < currentLevel; i++) {
            const neededForPrevLevel = 20 + (i - 1) * 10;
            trainingsInCurrentLevel -= neededForPrevLevel;
        }

        // Если отрицательное - обнуляем
        ...
        console.log(`📊 Прогресс: уровень ${currentLevel}, тренировок в этом уровне: ${trainingsInCurrentLevel}, нужно до след.уровня: ${trainingsNeededForNextLevel}, прогресс: ${progress}%`);
    ...
    function initWeightEdit() {
        ...
        weightBox.addEventListener('click', function () {
            ...
            if (newWeight && !isNaN(newWeight) && newWeight > 0 && newWeight < 300) {
                ...
                console.log('✅ Вес обновлен:', user.weight);
            } else if (newWeight) {
    ...
    function initWorkoutIncrement() {
        ...
        if (!trainingsBox) {
            console.warn('⚠️ Блок с тренировками не найден');
            return;
        }
        ...
        plusBtn.addEventListener('click', (e) => {
            ...
            // --- ЛОГИКА ПОЛУЧЕНИЯ НАГРАДЫ "НОВИЧОК" (5 ТРЕНИРОВОК) ---
            // --- ЛОГИКА ПОЛУЧЕНИЯ НАГРАДЫ "НОВИЧОК" (5 ТРЕНИРОВОК) ---
            ...
            console.log(`✅ Тренировка добавлена! Всего: ${user.stats.trainings}`);
        });
    }
    ...
    function loadUserProfile(user) {
        console.log('📥 Загрузка профиля для пользователя:', user);
    ...
    function showFitnessBandPrompt(user) {
        ...
        if ((!hasSteps && !hasCalories) && !hasSeenPrompt) {
            console.log('📱 Показываем блок подключения браслета');
            setTimeout(() => {
        ...
        function tryShowPrompt() {
            ...
            if (!trainingStatsRow) {
                ...
                if (attempts < maxAttempts) {
                    console.log(`⏳ Ждём загрузку блока тренировок... попытка ${attempts}`);
                    setTimeout(tryShowPrompt, 300);
                } else {
                    console.error('❌ Блок с тренировками не найден после нескольких попыток');
                }
                return;
            }
        ...
        `;
            ...
            if (connectBtn) {
                connectBtn.addEventListener('click', () => {
                    ...
                    // Сохраняем, что браслет уже предлагали
                    ...
                    setTimeout(() => {
                        ...
                        // Показываем уведомление об успешном подключении (без цифр)
                        showBandConnectionSuccessNotification();

                        // Не перезагружаем страницу, чтобы уведомление успели увидеть
                        // location.reload();
                    }, 300);
                });
            }
    ...
    `;
        ...
        // Принудительно показываем через setTimeout
        setTimeout(() => {
    ...
    function initAvatarUpload() {
        ...
        fileInput.addEventListener('change', (e) => {
            ...
            // Проверяем размер файла (если больше 2MB - сжимаем)
            if (file.size > 2 * 1024 * 1024) {
                ...
                reader.onload = (event) => {
                    compressImageFromDataUrl(event.target.result, 0.8, (compressedDataUrl) => {
                        saveAvatar(compressedDataUrl);
                    });
                };
    ...
    function compressImageFromDataUrl(dataUrl, quality, callback) {
        ...
        img.onload = () => {
            ...
            // Максимальные размеры для аватара (120x120)
            const maxWidth = 120;
            const maxHeight = 120;
            ...
            // Масштабируем под нужные размеры
            if (width > height) {
                if (width > maxWidth) {
            ...
            // Получаем сжатое изображение (JPEG, качество 0.7-0.8)
    ...
    function loadChallenges(user) {
        ...
        // Сортируем завершенные по дате (сначала новые)
        const sortedCompleted = [...completed].sort((a, b) =>
            new Date(b.completedAt) - new Date(a.completedAt)
        );

        // ПОКАЗЫВАЕМ ВСЕ АКТИВНЫЕ ЧЕЛЛЕНДЖИ
        if (active.length > 0) {
            active.forEach(c => list.appendChild(createChallengeElement(c, 'active')));
        }

        // ПОКАЗЫВАЕМ ВСЕ ЗАВЕРШЕННЫЕ ЧЕЛЛЕНДЖИ (без разделителя)
    ...
    function initGlobalSearch() {
        ...
        // ПРИ НАЖАТИИ ENTER - ПЕРЕНАПРАВЛЕНИЕ (показываем результат)
        searchInput.addEventListener('keypress', function (e) {
            if (e.key === 'Enter') {
                ...
                if (query.length >= 2) {
                    ...
                    // Если 1 символ - показываем сообщение
        ...
        // Очищаем значение при фокусе (опционально)
        searchInput.addEventListener('focus', function () {
            // Ничего не делаем, ждём Enter
        });
    }

    // Функция перенаправления при нажатии Enter
    function redirectToSearchResult(query) {
        ...
        // 4. НОВОЕ: Поиск в товарах магазина (shopItems)
        const shopItems = [
        ...
        // 5. НОВОЕ: Поиск в премиум контенте
    ...
    function completeDailyChallenge(challengeId) {
        ...
        // Сохраняем информацию об уведомлении о награде в sessionStorage
    ...
    function markChallengeDay(challengeId, keepModalOpen = true) {
        ...
        if (challenge.current >= challenge.total) {
            ...
            // Показываем уведомление о прогрессе (сразу, без перезагрузки)
            if (typeof window.showNotification === 'function') {
    ...
    // Универсальная функция добавления награды
    // Универсальная функция добавления награды
    function addAchievementToUser(user, achievementId, customName = null) {
        ...
        user.pointTransactions.push({
            ...
            points: achievementData.points,
            date: now.toISOString(), // ПОЛНАЯ ДАТА С ВРЕМЕНЕМ
            type: 'achievement'
        });
